...
async function main() {
  ...
  const relayTransactionHash2= await itx.send('relay_sendTransaction', [tx, signature]);
  console.log(`ITX relay transaction hash: ${relayTransactionHash2}`);
  ...
  while (true) {
    ...
    const statusResponse = await itx.send('relay_getTransactionStatus', [relayTransactionHash2]);
